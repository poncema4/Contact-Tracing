...
class UnifiedClient:
    ...
    def connect_to_server(self, host=None, port=12345):
        try:
            if not host:
            ...
            join_message = f"JOIN:{self.player_name}".encode()
    ...
    def get_server_ip(self):
        """Prompt user for server IP address with better default handling"""
        dialog = tk.Toplevel(self.root)
        dialog.title("Server Connection")
        dialog.transient(self.root)
        dialog.grab_set()
        
        ip_address = ['localhost']  # Default value
        
        # Center dialog
        dialog.geometry("300x150")
        dialog.resizable(False, False)
        
        ttk.Label(dialog, text="Enter server IP address:").pack(pady=10)
    ...
    def receive_messages(self):
        while True:
            try:
                ...
                if not message:
                    raise ConnectionResetError("Server connection lost")
                ...
                self.chat_display.configure(state='normal')
                self.chat_display.insert(tk.END, "*** Connection to server lost. Attempting to reconnect... ***\n", 'system')
                self.chat_display.configure(state='disabled')
                # Try to reconnect
                if self.connect_to_server(self.server_ip):
                    self.chat_display.configure(state='normal')
                    self.chat_display.insert(tk.END, "*** Reconnected to server ***\n", 'system')
                    self.chat_display.configure(state='disabled')
                    continue
                break
            except Exception as e:
    ...
    def handle_chat_input(self, event):
        ...
        if message:
            try:
                ...
                self.show_error(f"Failed to send message: {str(e)}")
                # Try to reconnect
                self.connect_to_server(self.server_ip)
